...
namespace CSCI441 {
  ...
  *
    ...
    */
    ...
    void drawSolidTopHemisphere( GLfloat radius, GLint stacks, GLint slices );
    void drawSolidBottomHemisphere( GLfloat radius, GLint stacks, GLint slices );
    void drawSolidSphere( GLfloat radius, GLint stacks, GLint slices );
...
namespace CSCI441_INTERNAL {
    ...
    void drawTopHemiSphere( GLfloat radius, GLint stacks, GLint slices, GLenum renderMode );
    void drawBottomHemiSphere( GLfloat radius, GLint stacks, GLint slices, GLenum renderMode );
...
inline void CSCI441::drawSolidTopHemisphere( GLfloat radius, GLint stacks, GLint slices ) {
    assert( radius > 0.0f );
    assert( stacks > 1 );
    assert( slices > 2 );

    CSCI441_INTERNAL::drawTopHemiSphere( radius, stacks, slices, GL_FILL );
}
inline void CSCI441::drawSolidBottomHemisphere( GLfloat radius, GLint stacks, GLint slices ) {
    assert( radius > 0.0f );
    assert( stacks > 1 );
    assert( slices > 2 );

    CSCI441_INTERNAL::drawBottomHemiSphere( radius, stacks, slices, GL_FILL );
}
...
/*
    for( int stackNum = 1; stackNum < (stacks)/2; stackNum++ ) {

        glDrawArrays( GL_TRIANGLE_STRIP, (slices+2) + (stackNum-1)*((slices+1)*2), (slices+1)*2 );
    }
*/
    glDrawArrays( GL_TRIANGLE_FAN, (slices+2) + (stacks-2)*(slices+1)*2, (slices+2)/ 2 );

    glPolygonMode( GL_FRONT_AND_BACK, GL_FILL );
}

inline void CSCI441_INTERNAL::drawTopHemiSphere( GLfloat radius, GLint stacks, GLint slices, GLenum renderMode ) {
    SphereData sphereData = { radius, stacks, slices };
    if( CSCI441_INTERNAL::_sphereVAO.find( sphereData ) == CSCI441_INTERNAL::_sphereVAO.end() ) {
        CSCI441_INTERNAL::generateSphereVAO( sphereData );
    }
    ...
    glDrawArrays( GL_TRIANGLE_FAN, 0, slices+2 );
    for( int stackNum = (stacks)/2; stackNum < stacks-1; stackNum++ ) {
...
inline void CSCI441_INTERNAL::drawBottomHemiSphere( GLfloat radius, GLint stacks, GLint slices, GLenum renderMode ) {
    SphereData sphereData = { radius, stacks, slices };
    if( CSCI441_INTERNAL::_sphereVAO.find( sphereData ) == CSCI441_INTERNAL::_sphereVAO.end() ) {
        CSCI441_INTERNAL::generateSphereVAO( sphereData );
    }
